...
### Priority 1 — Season ↔ Career loop integration (recommended next)
- Trigger recruiting progression from season-week advancement (single action loop).
- Surface coach program snapshot directly in season flow and vice-versa.
- Add clear prompts when either side is incomplete (no season started / no coach setup).
