...
import { TeamResume, computePlayoffProjection, computeRankings, computeResumeByTeam } from '../../sim/rankings';
...
export const startPlayoffs = createAsyncThunk<PlayoffState | null, void, { state: RootState }>('season/startPlayoffs', async (_arg, { getState }) => {
  ...
  const top12 = computePlayoffProjection(state.league.teams, season.recordsByTeamId, season.resultsByWeek);
...
const selectResultsByWeek = (state: RootState) => state.season.resultsByWeek;
...
export const selectResumeByTeam = createSelector(
  [selectTeams, selectRecordsByTeamId, selectResultsByWeek],
  (teams, recordsByTeamId, resultsByWeek): Record<string, TeamResume> => computeResumeByTeam(teams, recordsByTeamId, resultsByWeek),
);

export const selectTop25Rankings = createSelector([selectTeams, selectRecordsByTeamId, selectResultsByWeek], (teams, recordsByTeamId, resultsByWeek): RankingRow[] =>
  computeRankings(teams, recordsByTeamId, resultsByWeek, 25),
);

export const selectTop12Projection = createSelector([selectTeams, selectRecordsByTeamId, selectResultsByWeek], (teams, recordsByTeamId, resultsByWeek): RankingRow[] =>
  computePlayoffProjection(teams, recordsByTeamId, resultsByWeek),
);

export const selectNextFourOut = createSelector([selectTeams, selectRecordsByTeamId, selectResultsByWeek], (teams, recordsByTeamId, resultsByWeek): RankingRow[] =>
  computeRankings(teams, recordsByTeamId, resultsByWeek, 16).slice(12, 16),
);
