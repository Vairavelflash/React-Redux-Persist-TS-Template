...
import { selectResumeByTeam, selectTop12Projection, simNextPlayoffRound, selectPlayoffState, startPlayoffs } from '../features/season/seasonSlice';
...
function PlayoffsPage() {
  ...
  return (
    <section>
      ...
      {!playoffState && top12.length > 0 ? (
        <div className="card">
          <h3>Selection Sunday Snapshot</h3>
          <p className="mutedText">Projected field based on current resume. Seeds 1–4 receive byes to the quarterfinals.</p>
          <div className="selectionGrid">
            {top12.map((row) => {
              const team = teamById.get(row.teamId);
              const resume = resumeByTeam[row.teamId];
              const byeTag = row.rank <= 4 ? 'BYE' : 'R1';
              return (
                <article key={row.teamId} className="selectionCard">
                  <p className="selectionSeed">#{row.rank} · {byeTag}</p>
                  <p className="selectionTeam">{team?.schoolName} {team?.nickname}</p>
                  <p className="mutedText">
                    {row.record} · SOS {(resume?.sos ?? 0).toFixed(3)} · QW {resume?.qualityWins ?? 0}
                  </p>
                </article>
              );
            })}
